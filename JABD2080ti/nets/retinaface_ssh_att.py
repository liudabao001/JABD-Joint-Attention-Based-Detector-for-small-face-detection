...
class RetinaFace(nn.Module):
    def __init__(self, cfg=None, pretrained=False, mode='train'):
        ...
        #   利用ssh模块提高模型感受野
        # -------------------------------------------#
        self.eca_ = eca_block(cfg['out_channel'])
    ...
    def forward(self, inputs):
        ...
        # fpn[1]  (1,256,80,80)
        # fpn[2]  (1,256,40,40)
        # fpn[0] =self.eca(fpn[0])
        # fpn[1] =self.eca(fpn[1])
        # fpn[2] =self.eca(fpn[2])

        # self.ssh1 = self.eca_(self.ssh1(fpn[0]))
        # self.ssh1 = self.eca_(self.ssh1(fpn[1]))
        # self.ssh1 = self.eca_(self.ssh1(fpn[2]))

        feature1 = self.eca_(self.ssh1(fpn[0]))
        feature2 = self.eca_(self.ssh1(fpn[1]))
        feature3 = self.eca_(self.ssh1(fpn[2]))
