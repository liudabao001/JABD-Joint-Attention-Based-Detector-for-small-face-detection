...
from nets.resnet_pytorch import resnet152
from nets.layers import FPN_152, SSH,FPN_me,FPN_152_5
...
class RetinaFace(nn.Module):
    def __init__(self, cfg=None, pretrained=False, mode='train'):
        ...
        # layer2.register_forward_hook(self.hook_fn)
        # layer3.register_forward_hook(self.hook_fn)
        # layer4.register_forward_hook(self.hook_fn)
        ...
        #   利用初步有效特征层构建特征金字塔
        # -------------------------------------------#
        self.fpn = FPN_152_5(in_channels_list, cfg['out_channel'])
        # self.fpn = FPN_me(in_channels_list, cfg['out_channel'])
        # -------------------------------------------#
        #   利用ssh模块提高模型感受野
        # -------------------------------------------#
        ...
        self.ssh4 = SSH(cfg['out_channel'], cfg['out_channel'])
        self.ssh5 = SSH(cfg['out_channel'], cfg['out_channel'])
