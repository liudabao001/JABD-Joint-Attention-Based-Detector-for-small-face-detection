...
from nets.layers import FPN, SSH,FPN_me
...
class eca_block(nn.Module):
    def __init__(self, channel, b=1, gamma=2):
        ...
        self.harfsigmoid = nn.Hardsigmoid()
...
class RetinaFace(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # out1 = self.eca_64(out[0])
        # out2 = self.eca_128(out[1])
        # out3 = self.eca_256(out[2])
        # out = [out1,out2,out3]
        inputs = list(out.values())
        out1 = self.eca_64(inputs[0])
        out2 = self.eca_128(inputs[1])
        out3 = self.eca_256(inputs[2])
        out = [out1, out2, out3]
        # -------------------------------------------#
        #   获得三个shape的有效特征层
        #   分别是output1  80, 80, 64
