...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = "logs_moblieV3_r_ecaG_non1368_xiaoron/8/Epoch85-Total_Loss6.2689.pth"
    # -------------------------------------------------------------------#
    ...
    class NLM(nn.Module):
        def __init__(self, in_channels, scale=1, psp_size=(1, 3, 6, 8), ch=8):  # psp_size=(1, 3, 6, 8)(1, 4, 8, 12)
    ...
    class FPN(nn.Module):
        ...
        def forward(self, inputs):
            ...
            # up3 = self.pixelshuffle_block(40, 40, int(output1.size(2) / output2.size(3)))
            up3 = self.nlm(up3)
            output2 = output2 + up3
            ...
            # up2 = self.pixelshuffle_block(40, 40, int(output1.size(1) / output2.size(2)))
            up2 = self.nlm(up2)
            output1 = output1 + up2
    ...
    loss_history = LossHistory("logs_moblieV3_r_ecaG_non1368_xiaoron/8")
    ...
    def fit_one_epoch(model_train, model, loss_history, optimizer, criterion, epoch, epoch_step, gen, Epoch, anchors,
        ...
        if (epoch + 1) % save_period == 0:
            torch.save(model.state_dict(), 'logs_moblieV3_r_ecaG_non1368_xiaoron/8/Epoch%d-Total_Loss%.4f.pth' % (
