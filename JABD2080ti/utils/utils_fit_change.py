...
def fit_one_epoch(model_train, model, loss_history, optimizer, criterion, epoch, epoch_step, gen, Epoch, anchors, cfg,
    ...
    if (epoch + 1) % save_period == 0:
        torch.save(model.state_dict(), 'logs_50_4/Epoch%d-Total_Loss%.4f.pth' % (
        (epoch + 1), (total_c_loss + total_r_loss + total_landmark_loss) / (epoch_step + 1)), _use_new_zipfile_serialization=False)
