...
from nets.mobilenetV3 import MobileNetV3_Large_change,MobileNetV3_Large_eca
...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = "logs_moblieV3_r_ecaG_non1368_xiaoron/40/Epoch47-Total_Loss4.8734.pth"
    # -------------------------------------------------------------------#
    ...
    class NLM(nn.Module):
        def __init__(self, in_channels, scale=1, psp_size=(1, 3, 6, 8), ch=40):  # psp_size=(1, 3, 6, 8)(1, 4, 8, 12)
    ...
    class FPN(nn.Module):
        ...
        def forward(self, inputs):
            ...
            #   output2  40, 40, 64
            # -------------------------------------------#
            up3 = F.interpolate(output3, size=[output2.size(2), output2.size(3)], mode="bicubic",align_corners=True)
            up3 = self.nlm(up3)
            output2 = output2 + up3
            ...
            #   output1  80, 80, 64
            # -------------------------------------------#
            up2 = F.interpolate(output2, size=[output1.size(2), output1.size(3)], mode="bicubic",align_corners=True)
            up2 = self.nlm(up2)
            output1 = output1 + up2
    ...
    loss_history = LossHistory("logs_moblieV3_r_ecaG_non1368_bicubic")
    ...
    def fit_one_epoch(model_train, model, loss_history, optimizer, criterion, epoch, epoch_step, gen, Epoch, anchors,
        ...
        if (epoch + 1) % save_period == 0:
            torch.save(model.state_dict(), 'logs_moblieV3_r_ecaG_non1368_bicubic/Epoch%d-Total_Loss%.4f.pth' % (
