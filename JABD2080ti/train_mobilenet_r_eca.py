...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = "logs_mobilenetV3bolck_r_ecaG/Epoch93-Total_Loss6.0886.pth"
    # model_path = ""
    # -------------------------------------------------------------------#
    ...
    class NLM(nn.Module):
        def __init__(self, in_channels, scale=1, psp_size=(1, 3, 6, 8), ch=4):  # psp_size=(1, 3, 6, 8)
    ...
    class FPN(nn.Module):
        def __init__(self, in_channels_list, out_channels):
            ...
            # self.up_p = PixelShuffleBlock()
            # self.up_p = nn.PixelShuffle(2)
            # self.nlm = NLM(40)
            # self.nlm = NLM(64)
        ...
        def forward(self, inputs):
            ...
            #   output2  40, 40, 64
            # -------------------------------------------#
            # up3 = self.up_p(output3)
            up3 = nn.PixelShuffle(output2/output3)
            ...
            up2 = nn.PixelShuffle(output1/ output2)
            # up2 = self.nlm(up2)
            output1 = output1 + up2
    ...
    class RetinaFace(nn.Module):
        ...
        def forward(self, inputs):
            ...
            feature2 = self.ssh1( self.eca_fpn(fpn[1]))
            feature3 = self.ssh1( self.eca_fpn(fpn[2]))
    ...
    loss_history = LossHistory("logs_mobilenetV3bolck_r_ecaG")
    ...
    def fit_one_epoch(model_train, model, loss_history, optimizer, criterion, epoch, epoch_step, gen, Epoch, anchors,
        ...
        if (epoch + 1) % save_period == 0:
            torch.save(model.state_dict(), 'logs_mobilenetV3bolck_r_ecaG/Epoch%d-Total_Loss%.4f.pth' % (
